...
class DataCollatorForMatching:
    ...
    def __call__(self, samples: List[List[List[List[int]]]]) -> Dict[str, torch.Tensor]:
        ...
        for i, sample in (enumerate(samples)):
            if self.is_label:
                input_ids_query_and_neighbors, attention_mask_query_and_neighbors, mask_query_and_neighbors = self.create_training_sample(
                    sample, self.is_label)
            else:
    ...
    def create_training_sample(self, sample: List[List[List[int]]], is_label=False):
        ...
        def process_node_and_neighbors(tokens_node_and_neighbors):
            ...
            # This could be a potential error here, as we are not sampling neighbours randomly and only picking the first five
            # and putting in the training list.
        ...
        if is_label:
            tokens_query_and_neighbors = sample
