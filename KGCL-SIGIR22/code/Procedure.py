...
def Validation(dataset, Recmodel, epoch, w=None, multicore=0):
    u_batch_size = world.config['test_u_batch_size']
    dataset: utils.BasicDataset
...
def Test(dataset, Recmodel, epoch, w=None, multicore=0, test_save_path=None):
    u_batch_size = world.config['test_u_batch_size']
    dataset: utils.BasicDataset
    testDict: dict = dataset.testDict
    Recmodel: model.KGCL
    ...
    with torch.no_grad():
        ...
        if test_save_path:
            torch.save([users_list, rating_list, groundTrue_list], test_save_path)
