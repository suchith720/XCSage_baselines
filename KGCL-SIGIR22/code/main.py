...
weight_file = utils.getFileName(type="weight")
output_file = utils.getFileName(type="output")
...
try:
    ...
    for epoch in tqdm(range(world.TRAIN_epochs), disable=True):
        ...
        if not world.pretrain_kgc:
            ...
            if world.kgc_joint or world.uicontrast!="NO":
                output_information = Procedure.BPR_train_contrast(dataset, Recmodel,
                                                                  bpr, contrast_model,
                                                                  contrast_views,
                                                                  epoch, optimizer,
                                                                  neg_k=Neg_k,w=w)
            ...
            if epoch<world.test_start_epoch:
                if epoch %5 == 0:
                    ...
                    Procedure.Validation(dataset, Recmodel, epoch, w, world.config['multicore'])
            else:
                if epoch % world.test_verbose == 0:
                    cprint("[VALIDATION]\n")
                    # result = Procedure.Test(dataset, Recmodel, epoch, w, world.config['multicore'])
                    result = Procedure.Validation(dataset, Recmodel, epoch, w, world.config['multicore'])
    ...
    #endfor
    print("[TEST]")
    result = Procedure.Test(dataset, Recmodel, epoch, w, world.config['multicore'], output_file)
    print(result)
